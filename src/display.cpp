...
void Display::update_window_scanline(Byte current_scanline)
{
    ...
    for (int x = 0; x < 160; x++)
    {
        ...
        else
        {
            ...
            update_window_tile_pixel(palette, display_x, display_y, tile_x_pixel, tile_y_pixel,
                                     tile_id);
        }
    }
}

void Display::update_bg_tile_pixel(Byte palette, int display_x, int display_y, int tile_x,
                                   int tile_y, Byte tile_id)
{
    ...
    Byte high = memory->read(offset + (tile_y * 2) + 1), low = memory->read(offset + (tile_y * 2));

    sf::Color color = get_pixel_color(palette, low, high, tile_x, false);
    bg_array.setPixel(display_x, display_y, color);
}

void Display::update_window_tile_pixel(Byte palette, int display_x, int display_y, int tile_x,
                                       int tile_y, Byte tile_id)
{
...
void Display::render_sprite_tile(Byte palette, int start_x, int start_y, Byte tile_id, Byte flags)
{
    ...
    for (int y = 0; y < 8; y++)
    {
        ...
        Byte high = memory->read(offset + (y * 2) + 1), low = memory->read(offset + (y * 2));
...
sf::Color Display::get_pixel_color(Byte palette, Byte top, Byte bottom, int bit, bool is_sprite)
{
    ...
    {
    case 0x0:
        return (is_sprite) ? sf::Color::Transparent : shades_of_gray[color_0_shade];
    case 0x1:
        return shades_of_gray[color_1_shade];
    case 0x2:
        return shades_of_gray[color_2_shade];
    case 0x3:
        return shades_of_gray[color_3_shade];
    default:
        return sf::Color(255, 0, 255); // error color
    }
}
